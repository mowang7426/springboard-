...
#pragma mark - 1. 私有类及底层服务原生接口声明 (终结卡顿与 Safe Mode 核心)

// 👑 标准 FBS 接口：告别黑魔法，完美接管系统底层跳转，0卡顿、防崩溃！
        ...
        if (combinedModeVisible) {
            ...
            if (targetReq) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    id userInfo = nil;
                    @try {
                        id userNotif = [reqObj respondsToSelector:@selector(userNotification)] ? [(id<SBCPUNativeCallDummy>)reqObj userNotification] : nil;
                        userInfo = [userNotif respondsToSelector:@selector(userInfo)] ? [(id<SBCPUNativeCallDummy>)userNotif userInfo] : nil;
                        if (!userInfo) {
                            id bulletin = [reqObj respondsToSelector:@selector(bulletin)] ? [(id<SBCPUNativeCallDummy>)reqObj bulletin] : nil;
                            userInfo = [bulletin respondsToSelector:@selector(userInfo)] ? [(id<SBCPUNativeCallDummy>)bulletin userInfo] : nil;
                        }
                    } @catch (NSException *e) {}
...
- (void)expandFromEdgeAnimated:(BOOL)animated {
    ...
    // 🟢 展开时，先解除强制物理隐藏，再交由 updateLayout 梳理状态
    for (UIView *v in self.performanceContainer.subviews) {
